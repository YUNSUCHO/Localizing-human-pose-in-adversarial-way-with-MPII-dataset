...
parser.add_argument('--train_split', type=float, default=.999)
...
class MPII(data.Dataset):
	def __init__(self, split, cfg):
		...
		annot = {}
		for tag in tags:
			annot[tag] = np.asarray(f[tag]).copy()[0:11000] #need to comment out when test the model
...
#print("the value of pts :" , pts) 
	...
	def __len__(self):
        # Return length
		if self.mode == 'train':
			return int(self.train_split * len(self.annot['scale']))
		else:
			return len(self.annot['scale']) - int(self.train_split * len(self.annot['scale']))     
	...
	def __getitem__(self, index):
		...
		if self.mode == 'val':
			...
			index += int(self.train_split * len(self.annot['scale']))
...
#--------------------------------------------------------------------------------------------------------------
			...
			#print(out_s.shape())
#		dis = np.linalg.norm(pts[8] - pts[9])
		print(time.process_time() - start)
		return {
...
#		plt.imshow(data['original'].numpy().transpose(1, 2, 0)[:, :, ::-1] + 0)
		plt.subplot(1, 4, 2)
		###plt.imshow(data['image'].transpose(1, 2, 0)[:, :, ::-1] + 0) ## orginal line replaced by below line
