...
class PCK(object):
    ...
    def calc_dists(self, preds, target, normalize, edistance=None):
        preds = preds.astype(np.float32) #preds = predicted coordinate value
        target = target.astype(np.float32) #target = ground truth cordinate value
        dists = np.zeros((preds.shape[1], preds.shape[0]))
        # n = batch, c= data index number
        for n in range(preds.shape[0]):
            for c in range(preds.shape[1]):
                #------------------------------------------------------
                if target[n, 8, 0] > 1 and target[n, 9, 0] > 1: #>1 to make sure the co-ordinates are present, if it is <1, it means the co-ordinate are not presented  
                    head_top  =target[n, 8, :2] 
                    head_low  =target[n, 9, :2]
                    normalize   = np.linalg.norm(head_top - head_low)
    ...
    def dist_acc(self, dists, thr=0.5):
         ...
         #print('total number of key point present', dist_cal.sum())
