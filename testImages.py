...
#from datasets.mpii_trainingaccuracy import MPII
...
parser.add_argument('--path', \
    default='/home/aburagohain/Adversarial-Pose-Estimation/lspet_dataset')
...
with torch.no_grad():
    ...
    #generator_model  = PoseNet(nstack = 8 , inp_dim = 3, oup_dim = 16)
    ...
    # Load
    #generator_model = generator_model.load_state_dict(torch.load(args.modelName)['state_dict'])
    ...
    lsp_train_dataset = MPII('train')#LSP(args)
    args.mode = 'val'    
    lsp_val_dataset =MPII('valid')   #MPII('valid') originally it was  "val" in place of "Valid" LSP(args)  
    ...
    for i, data in tqdm(enumerate(val_eval_loader)):
        ...
        if mean_eval_avg_acc > 0.99:    ##original value is > 0.8 #reason for change is the val =0.00, wanted to check the results
            plt.clf()
            ...
            idx = np.argmax(ground_truth['occlusions'].detach().cpu().numpy()[0].reshape(16, -1), 1) # 
            y1 = idx/62
            x1 = idx%64
            ...
            idx = np.argmax(outputs[-1][:, :16, :, :].detach().cpu().numpy()[0].reshape(16, -1), 1) # originally it was 16 in place of 14
            y = idx/64
            x = idx%64
            ...
            plt.savefig('testresults/mpii11000-adversarial(86.43%)-best-1/results_{}.png'.format(i))
                             ###needs to be modified accordinf to the model testing
            plt.show()
