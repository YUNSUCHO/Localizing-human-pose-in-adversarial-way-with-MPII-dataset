...
from datasets.mpii_valid import MPII
...
with torch.no_grad():
    ...
    args.mode = 'train'    
    lsp_val_dataset =MPII('train')   #MPII('valid') originally it was  "val" in place of "Valid" LSP(args)  
    ...
    for i, data in tqdm(enumerate(val_eval_loader)):
        ...
        if mean_eval_avg_acc > 10000:    ##original value is > 0.8 #reason for change is the val =0.00, wanted to check the results
            plt.clf()
            ...
            plt.savefig('testresults/results_{}.png'.format(i))
