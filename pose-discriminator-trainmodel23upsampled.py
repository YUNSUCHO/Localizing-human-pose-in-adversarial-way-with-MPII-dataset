...
from datasets.mpii11000 import MPII
...
discriminator_model_pose = Discriminator(16 + 3 , config['discriminator']['num_channels'], config['dataset']['num_joints'], config['discriminator']['num_residuals'])

modelpath = torch.load('mpii-pretrain-high-accuracy/model_100_1300.pt') ##-------------->
...
for epoch in range(args.epochs):
    ...
    ########################  Check and complete the code here ######## Forward pass and calculate losses here ################
        ...
        pose_real.append(float(pose_real_loss))
        pose_fake.append(float(pose_fake_loss))
       ...
       # epoch_disc_loss += cur_disc_loss
        ...
        if i % args.print_every == 0:
            ...
            plt.savefig('trainingImages/train_output_exp23_pose.png')
            
            
            plt.clf()
            plt.imshow(np.sum(ground_truth['heatmaps'].detach().cpu().numpy()[0], axis=0))
            plt.savefig('trainingImages/train_gt_exp23_pose.png')
            ...
            plt.savefig('trainingImages/train_img_exp23_pose.png')
        # Save model
        ######################################################################################################################
        if i > 0 and i % 10000 == 0:
            ...
            torch.save({'discriminator_model_pose': discriminator_model_pose,
                        ...
                        os.path.join(args.modelName, 'model_pretrained_pose_{}_{}.pt'.format(epoch, i)))
